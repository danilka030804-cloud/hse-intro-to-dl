...
def training_epoch(model, optimizer, criterion, loader, tqdm_desc: str):
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        attention_mask = create_attention_mask(indices, lengths)
        optimizer.zero_grad()
        outputs = model(
            input_ids=indices,
...
def create_attention_mask(indices: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
    """
    Создать attention mask по длинам:
    1 — реальные токены, 0 — паддинги.
    indices: (batch, seq_len)
    lengths: (batch,)
    """
    batch_size, seq_len = indices.size()
    mask = torch.arange(seq_len, device=indices.device).expand(batch_size, seq_len)
