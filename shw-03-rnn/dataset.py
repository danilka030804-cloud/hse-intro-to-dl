...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        # если длина > max_length, обрезаем, но гарантируем <eos> в конце
        if len(indices) > self.max_length:
            indices = indices[:self.max_length - 1] + [self.eos_id]

        length = len(indices)

        # паддинг
        if len(indices) < self.max_length:
            indices.extend([self.pad_id] * (self.max_length - len(indices)))

        # теперь длина всегда ровно self.max_length
        assert len(indices) == self.max_length, f"Got {len(indices)}, expected {self.max_length}"
